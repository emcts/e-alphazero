...
def loss_fn(model_params, model_state, samples: Sample):
    (logits, value), model_state = forward.apply(model_params, model_state, samples.obs, is_eval=False)
...
def train(model, opt_state, data: Sample):
    model_params, model_state = model
    grads, (model_state, policy_loss, value_loss) = jax.grad(loss_fn, has_aux=True)(model_params, model_state, data)
...
def evaluate(rng_key, my_model):
    ...
    def body_fn(val):
        key, state, R = val
        (my_logits, _), _ = forward.apply(my_model_params, my_model_state, state.observation, is_eval=True)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Shuffle samples and make mini batches
        ...
        for i in range(num_updates):
            mini_batch: Sample = jax.tree_util.tree_map(lambda x: x[i], mini_batches)
            model, opt_state, policy_loss, value_loss = train(model, opt_state, mini_batch)
